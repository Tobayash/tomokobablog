...
### 1. Generic Formulas for Simple Slopes of X on Y at High and Low of Z and W
  ...
  + 通常、ZとWの高低の値は変数の平均値から1SDを上か下の値とされる (Aiken & West, 1991)。それぞれ$Z_{H},Z_{L},W_{H},W_{L}$とする。 
  
  + 上記の式を下記のように書き直した場合、1つ目のセットはXに対するYのグラフの切片を表し、2つ目のセットのカッコ内は傾きを表す。
  $$
  Y=(b_{0}+b_{2}Z+b_{3}W+b_{6}ZW)+(b_{1}+b_{4}Z+b_{5}W+b_{7}ZW)X+\epsilon
  $$
  
  + これにより、4本の回帰線の傾きは次のように表すことができる。
